            ...
            public class ClashDetectionTests : MakeCarBookingTests
            {
                ...
                public static IEnumerable  BookingServiceClashDetectionPositiveTestCases
                {
                    get
                    {
                        ...
                        // No existing Car Bookings.
                        var testCase1 = new BookingServiceClashDetectionTestCaseData(_smallCar, 0, 1, "Small Car for 1 day from today", dateTimeToday);
                        yield return new TestCaseData(testCase1);

                        // No existing Bookings for the same Car.
                        ...
                        var testCase2 = new BookingServiceClashDetectionTestCaseData(_mediumCar, 0, 1, "Medium Car for 1 day", dateTimeToday, existingSmallCarBookingForToday, existingLargeCarBookingForToday, existingLuxuryCarBookingForToday);
                        yield return new TestCaseData(testCase2);

                        // No existing Bookings for the same Car on the same day.
                        ...
                        var testCase3 = new BookingServiceClashDetectionTestCaseData(_mediumCar, 0, 1, "Medium Car for 1 day", dateTimeToday, existingSmallCarBookingForToday, existingSmallCarBookingEndsTwoDaysAgo, existingSmallCarBookingStartsInTwoDays);
                ...
                public static IEnumerable  BookingServiceClashDetectionNegativeTestCases
                {
                    get
                    {
                        ...
                        // Existing Booking for the same Car.
                        // e.g. Requested from 15Jul to 16Jul, but Booking exists for 15Jul to 16Jul.
                        var existingLuxuryCarBookingForToday = new Booking() { CarId = _luxuryCar.Id, Name = "Luxury for 1 day from today", RentalDate = dateTimeToday.Date, ReturnDate = dateTimeTomorrow };
                        var testCase1 = new BookingServiceClashDetectionTestCaseData(_luxuryCar, 0, 1, "Luxury Car for 1 day", dateTimeToday, existingLuxuryCarBookingForToday);
                        yield return new TestCaseData(testCase1);

                        // Existing Booking for the same Car is currently with a customer.
                        // e.g. Requested from 15Jul to 16Jul, but Booking exists for 14Jul to 16Jul.
                        ...
                        var testCase3 = new BookingServiceClashDetectionTestCaseData(_largeCar, 0, 1, "Large Car for 1 day", dateTimeToday, existingLargeCarBookingForTwoDaysFromYesterday);
                        yield return new TestCaseData(testCase3);

                        // Existing Booking for the same Car needs Valeting.
                        // e.g. Requested from 15Jul to 16Jul, but Booking exists for 14Jul to 15Jul.
                        ...
                        var testCase2 = new BookingServiceClashDetectionTestCaseData(_mediumCar, 0, 1, "Medium Car for 1 day", dateTimeToday, existingMediumCarBookingForYesterday);
                        yield return new TestCaseData(testCase2);

                        // Existing Booking for the same Car is booked by a customer.
                        // e.g. Requested from 15Jul to 17Jul, but Booking exists for 16Jul to 17Jul.
                        ...
                        var testCase4 = new BookingServiceClashDetectionTestCaseData(_smallCar, 0, 2, "Small Car for 2 days", dateTimeToday, existingSmallCarBookingForTomorrow);
                        yield return new TestCaseData(testCase4);

                        // Existing Booking for the same Car needs Valeting, but it is booked by a customer.
                        // e.g. Requested from 15Jul to 16Jul, but Booking exists for 16Jul to 17Jul.
                        ...
                        var testCase5 = new BookingServiceClashDetectionTestCaseData(_luxuryCar, 0, 1, "Luxury Car for 1 day", dateTimeToday, existingLuxuryCarBookingForTomorrow);
                        yield return new TestCaseData(testCase5);

                        // Existing Booking for the same Car is currently with a customer.
                        // e.g. Requested from 15Jul to 16Jul, but Booking exists for 14Jul to 17Jul.
                        ...
                        var testCase6 = new BookingServiceClashDetectionTestCaseData(_mediumCar, 0, 1, "Medium Car for 1 day", dateTimeToday, existingMediumCarBookingForThreeDaysFromYesterday);
                ...
                {
                    public BookingServiceClashDetectionTestCaseData(Car car, float discount, int duration, string name, DateTime startDate, params Booking[] existingCarBookings)
                    {
                        Car = car;
                        Discount = discount;
                        Duration = duration;
